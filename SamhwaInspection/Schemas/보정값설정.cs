...
namespace SamhwaInspection.Schemas
{
    ...
    public class 보정값설정 : List<보정값변수>
    {
        ...
        private List<보정값변수> Load()
        {
            ...
            return JsonConvert.DeserializeObject<List<보정값변수>>(File.ReadAllText(마스터데이터파일), IvmUtils.Utils.JsonSetting());
        }
    ...
    public class 보정값변수
    {
        ...
        public void 플로우설정()
        {
            ...
            else if (this.변수명칭.Contains("Flow5"))
                this.플로우 = Flow구분.Flow5;
            else if (this.변수명칭.Contains("Flow6"))
                this.플로우 = Flow구분.Flow6;
        }
