...
namespace SamhwaInspection.Utils
{
    class CCLink
    {
        ...
        public  short nStationNo;                                
        public ushort nDeviceType;                              
        public  short nDeviceNo;                                 
        private short nSize;                                    
        ...
        {
            bool bError;
            bError = mdOpen(nChannel, nStationNo, out nPath) > 0;
            return bError;
        }
        ...
        public bool dataSendBit(Int32 path, short sno, short devNo, bool data)
        {
            bool bSuccess;
            if (data)
                bSuccess = mdDevSet(path, sno, DEVTYPE_B, devNo) > 0;
            else
                bSuccess = mdDevRst(path, sno, DEVTYPE_B, devNo) > 0;
            return bSuccess;
        }
        ...
        public bool dataReceive(Int32 path, short sno, short devType, short devNo, short size, short[] revBuf)
        {
            ...
            if (devType == DEVTYPE_W || devType == DEVTYPE_D)
                size *= 2;
            bSuccess = mdDevSet(path, sno, devType, devNo) > 0;
            bSuccess = mdReceive(path, sno, devType, devNo, ref size, ref revBuf[0]) > 0;
            return bSuccess;
        }
